...
VBOTeapot::VBOTeapot(int grid, mat4 lidTransform)
{
    ...
    // 정점 데이터 VBO 설정
    glNamedBufferData(handle[0], verts * 3 * sizeof(float), v, GL_STATIC_DRAW);
    glVertexArrayVertexBuffer(vaoHandle, 0, handle[0], 0, sizeof(glm::vec3));

    // 색상 데이터 VBO 설정
    glNamedBufferData(handle[1], verts * 3 * sizeof(float), n, GL_STATIC_DRAW);
    glVertexArrayVertexBuffer(vaoHandle, 1, handle[1], 0, sizeof(glm::vec3));

    glNamedBufferData(handle[2], verts * 2 * sizeof(float), tc, GL_STATIC_DRAW);
    glVertexArrayVertexBuffer(vaoHandle, 2, handle[2], 0, sizeof(glm::vec2));
